...
export default function FormAddEvent({ showModal, setShowModal, selectedDate, toast }) {
    const {
        data,
        setData,
        post,
    ...
    // to handle selected data when start to click specific date
    useEffect(()=>{
        if(selectedDate){
            setData({
                ...data,
                ["start_date"]: dateconvert(selectedDate),
            });
        }
